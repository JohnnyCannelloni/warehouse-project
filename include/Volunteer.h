...
class Volunteer {
    ...
    protected:
        int _completedOrderId; //Initialized to NO_ORDER if no order has been completed yet
        int _activeOrderId; //Initialized to NO_ORDER if no order is being processed
        string _type;
...
class CollectorVolunteer: public Volunteer {
    ...
    private:
        const int _coolDown; // The time it takes the volunteer to process an order
        int _timeLeft; // Time left until the volunteer finishes his current order
};
...
class LimitedCollectorVolunteer: public CollectorVolunteer {
    ...
    private:
        const int _maxOrders; // The number of orders the volunteer can process in the whole simulation
        int _ordersLeft; // The number of orders the volunteer can still take
};
...
class DriverVolunteer: public Volunteer {
    ...
    private:
        const int _maxDistance; // The maximum distance of ANY order the volunteer can take
        const int _distancePerStep; // The distance the volunteer does in one step
        int _distanceLeft; // Distance left until the volunteer finishes his current order
};
